...
## Other Variable Types in R | Special Values -- NA, Inf, NaN
<ol type="1">
...
c(T, TRUE, F, FALSE)
# What happens when multiple data types are combined in a single vector?
c(TRUE, FALSE, NA, Inf)
...
<p style="font-size:20px">Use `c()` or <a href="https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/append">`append()`</a> function</p>
```{r}
...
rbind(x, y)
```
<p class="pnote"><b>NOTE</b>: There are functions a few pkgs that does the same job -- e.g. <a href="https://cran.r-project.org/web/packages/plyr/plyr.pdf">`plyr::rbind.fill`</a><br><br>
...
<p style="font-size:20px">Similar to vector but there are two conditions</p>
...
<li>Index</li>
```{r}
mdat[2, 1, drop=F] # row 2 and col 1; drop=F to keep the matrix str
...
## Matrix | How to modify a Matrix
<ol start="4">
<li>matrix addition and mult</li>
```{r}
...
```{r}
<a href="https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/diag">`diag()`</a>
diag(3)                   # 3x3 identity matrix
diag(matrix(1:9, nrow=3)) # Diagonal elements of a matrix
upper.tri(); lower.tri()
t()
...
```{r}
vector("list", length=3) # Empty list of length 3
# Note: list is also called a generic vector
```

<li>Creating a list with <a href="https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/list">`list()`</a> function</li>
```{r}
list(vect=c(1,2,3), another_list=list(a=c("apple", "avocado"), b=1:5), mat=matrix(c(1,2,3,4), ncol=2), df=data.frame(v1=1:3, v2=c("a", "b", "c")))
```
</ol>
