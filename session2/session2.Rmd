...
```{r eval=T, comment=""}
# Named vectors
named_vec=c(eastern="Florida", central="Iowa",
...
UF_bldg[c("STZ", "HVNR")]
```
<li>Other <a href="https://stat.ethz.ch/R-manual/R-devel/library/base/html/sets.html">set operations</a></li>
<p class="pnote"><b>Note</b>: <a href="https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/subset">`subset()`</a> function</p>
</ol>
...
<p style="font-size:18px">Also check <a href="https://www.rdocumentation.org/packages/photobiology/versions/0.10.6/topics/na.omit">`na.omit()`</a> function</p>
```{r include=F}
# Answer
NA_vec[!is.na(NA_vec)]
...
y<-factor(x)    # ordered() if ordinal
y               # Levels are automatically set in an alphabetical order
...
```{r}
as.numeric(as.character(y)) # Solution: Convert to character first!
```
</ol>
</ol>
...
ISOdate(year="2021", month="09", day="23") # Also has arguments for...
Lubridate
```
